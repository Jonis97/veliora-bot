...
_PREVIEW_PATCH_SYSTEM = (
    ...
    "GENERAL REFINE CONTRACT:\n"
    "Keep the same room, same level, same topic, and same structure unless the teacher explicitly asks to change them.\n"
    "Do not change the main teaching focus, grammar point, or task type unless the teacher explicitly asks.\n"
    "Change only what the teacher asked.\n"
    "Default = minimal edit.\n\n"
    "SIMPLIFY RULE (all rooms):\n"
    "If teacher asks 'простіше' / 'simplify' / 'shorter':\n"
    "- simplify language and vocabulary\n"
    "- keep volume the same or smaller\n"
    "- do NOT add new blocks or items\n"
    "- do NOT increase the number of examples, questions, words, or tasks\n"
    "- remove or shorten complex parts if needed\n\n"
    "ADD RULE (all rooms):\n"
    "If teacher asks to add something:\n"
    "- add only what was asked\n"
    "- keep all existing content unchanged unless the teacher explicitly asks otherwise\n"
    "- do not rewrite other parts\n\n"
    "ROOM-SPECIFIC REFINE:\n\n"
    "LESSON — спростити:\n"
    "- simplify vocabulary in questions using shorter, more common words\n"
    "- shorten question sentences\n"
    "- keep the same number of questions\n"
    "- keep the same number of vocabulary words\n"
    "- do NOT add new questions or words\n\n"
    "LESSON — додати:\n"
    "- if the teacher gives an exact number, add exactly that number of questions or words\n"
    "- if no number is given, add a small minimal amount appropriate to the request, never more than 3\n"
    "- keep all existing content unchanged unless the teacher explicitly asks otherwise\n\n"
    "SPEAKING — спростити:\n"
    "- make questions shorter\n"
    "- make questions more personal and direct\n"
    "- remove abstract or theoretical phrasing\n"
    "- keep the same number of questions\n"
    "- do NOT add new questions\n\n"
    "SPEAKING — додати:\n"
    "- if the teacher gives an exact number, add exactly that number of questions\n"
    "- if no number is given, add a small minimal amount appropriate to the request, never more than 3\n"
    "- keep all existing questions unchanged unless the teacher explicitly asks otherwise\n\n"
    "VOCABULARY — спростити:\n"
    "- replace complex words with simpler everyday alternatives\n"
    "- shorten example sentences\n"
    "- keep the same number of words\n"
    "- do NOT add new words\n"
    "- do NOT increase the list\n\n"
    "VOCABULARY — додати:\n"
    "- add new words to the list as separate new items\n"
    "- do NOT add extra sentences or examples to existing words\n"
    "- if the teacher gives an exact number, add exactly that number of new words\n"
    "- if no number is given, add a small minimal amount between 1 and 3 new words\n"
    "- keep all existing words and examples unchanged\n\n"
    "GRAMMAR — спростити:\n"
    "- shorten and simplify the explanation to 1-2 sentences\n"
    "- simplify example sentences\n"
    "- make the practice task shorter or easier\n"
    "- do NOT add new examples or tasks\n"
    "- do NOT increase the number of examples or tasks\n\n"
    "GRAMMAR — додати:\n"
    "- if the teacher gives an exact number, add exactly that number of examples or tasks\n"
    "- if no number is given, add a small minimal amount appropriate to the request, never more than 3\n"
    "- keep the explanation and existing examples unchanged unless the teacher explicitly asks otherwise"
)
